...
자막           <video>.subs.json ─► 원본 언어 트랙 하나 ─► 귀속 판정 ─► 프롬프트 렌더 시 교정
                                                        ├─ 음성 전사  ─► 음성 인덱스
                                                        └─ 하드섭 사본 ─► 텍스트 인덱스
...
**교정은 저장된 인덱스가 아니라 프롬프트를 만들 때 적용됩니다.** `index.json`과 캐시의
두 `.txt`는 영상을 처리해서 나온 것만 담습니다 — 자막은 영상과 함께 도착한 별개의 것이고,
그걸 인덱스에 접어 넣으면 트랙이 생기거나 임계값 하나 움직일 때마다 VAD·ASR·OCR을 통째로
다시 돌게 됩니다. 밀리초짜리 교정을 위해 몇 분을 쓰는 셈입니다. 인덱스는 영상과 처리
설정만의 함수로 남고, 자막 트랙이 바뀌어도 캐시는 유효합니다.

실제로 보낸 프롬프트는 `runs/<run-id>/prompt.txt`에 남습니다. 인덱스는 질의를 거듭할수록
좋아질 예정이므로(에이전트가 부른 Whisper 결과가 누적됩니다), 캐시의 최신본보다 **이번
실행에 무엇을 보냈는지**가 재현에 필요합니다.

출력 형식은 바뀌지 않습니다. 출처도 등급도 신뢰도 표시도 프롬프트에 나타나지 않으며,
판정 결과와 융합 통계는 trace에만 남습니다. 에이전트는 여전히 무엇이 들렸는지를 받지,
누가 들었는지를 받지 않습니다.
...
`transcribe_segment`가 돌려준 문장은 그 실행과 함께 사라지고 있었습니다. 도구가 답을
요청 구간의 정확한 해시로만 캐시해서, 5초 옆을 다시 물으면 아무것도 재사용되지 않았고
...
한 행은 문장 하나이고, 그 문장이 속한 VAD region과 호출이 실제로 들은 **클립 범위**를
함께 적습니다. 같은 region을 두 번 전사했을 때 어느 쪽을 믿을지가 이 둘의 차이로
정해집니다 — region 전체를 들은 클립은 문장 앞의 조수를 들었고, 잘린 클립은 못 들었기
때문입니다. 덮은 비율이 같으면 나중 호출을 택합니다.

적용은 **자막보다 먼저**입니다.

```
저장된 인덱스   Hi everyone, I'm Maya Rarick ... at Central Trovva
+ 누적 Whisper  Hi everyone, I'm Maja Drabczuk ... at Centrum Cyfrowe
+ 자막 (최종)   Hi everyone, I'm Maja Drabczyk ... at Centrum Cyfrowe
```

채널이 올린 자막은 닿는 곳에서 여전히 이기고 — Whisper의 오타 `Drabczuk`까지 고칩니다 —
닿지 않는 곳은 에이전트가 이미 값을 치른 전사가 채웁니다. 질의를 거듭할수록 다음 질의가
받는 인덱스가 좋아집니다.

에이전트에게는 어느 구간이 이미 처리됐는지 알리지 않습니다. 무엇이 들렸는지를 받지,
누가 들었는지를 받지 않는다는 원칙은 그대로입니다.
...
`index.json`은 영상을 처리한 결과만 담습니다. 채널 메타데이터와 자막처럼 파일과 함께
도착한 것은 프롬프트를 만들 때 얹으며, 인덱스에 섞지 않습니다. (언어 힌트는 아직 예외입니다 —
OCR 인식기 선택에 쓰이므로 인덱스를 만들 때 필요합니다.)
